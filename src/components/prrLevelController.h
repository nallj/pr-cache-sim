#ifndef _PRR_LEVEL_CONTROLLER_H_
#define _PRR_LEVEL_CONTROLLER_H_
...
class prrLevelController {
	...
	public:
		//  applicationScheduler& as,
		//prrLevelController(unsigned long long& ccc, storageUnit*& prrs,
		...
		//		next_search_queue_(&nsq), prr_contents_(pc), next_state_(PRR_INIT) {}

		prrLevelController(unsigned prr_id, reconfigurableRegions* memory_hierarchy_top,
				std::vector<storageUnit*>* memory_hierarchy, double sim_speed) :
			prr_id_(prr_id), prr_(memory_hierarchy_top), memory_hierarchy_(memory_hierarchy),
			sim_speed_(sim_speed), next_state_(PRR_INIT), prr_executing_(false),
			icap_ack_(false), prc_ack_(false) {}
		...
		void connect(bool *prc_start, bool *prc_enqueue, unsigned *icap_mc, bool *icap_req, bool *icap_trans,
				traceToken** prc_current_trace_ptr, traceToken** icap_current_trace_ptr) {
				...
				switch (current_state_) {
					...
					case PRR_IDLE:
						...
						// module is already present and all that is needed is a simple run request.
							...
							if (prr_content->getModuleState() == VACANT)
								...
								region_id = prc_current_trace_->getRegionId();
						...
						// only accept a transfer requests from the ICAP if there are no other requests
							...
							region_id = icap_current_trace_->getRegionId();
							module_id = icap_current_trace_->getModuleId();
					...
					case PRR_TRANSFER:
						...
						if (!*icap_trans_) {
							...
							memoryLevel* memory_level = dynamic_cast<memoryLevel*>(memory_hierarchy_->at(*icap_mc_));

							module* transferred_module = memory_level->getModule(region_id, module_id);

							//transferred_module->spillGuts();
							prr_->insertModule(transferred_module);

							unsigned exe_duration = icap_current_trace_->getExectionTime();
	...
	private:
		...
		/*/unsigned long long* current_clock_cycle_;

		//applicationScheduler* application_scheduler_;

		//storageUnit* prr_level_;

		//unsigned trace_pointer_ = 0;

		//std::vector<std::queue< std::pair<unsigned, unsigned> >>* prr_action_queue_; // trace id, waiting level

		//std::queue<unsigned>* next_search_queue_; // trace id

		//std::pair<unsigned, bool>* prr_contents_; // trace id, currently executing (false = IDLE)

		//std::ostream pending_view_;


		//void checkForTraceRequest();*/


		// IN signals
		//bool *en_, *prr_ctrl_execute_, *icap_ack_, *icap_prog_;
		...
		// OUT signals
		//prrControllerState *prr_ctrl_status_;
		//bool *icap_enqueue;
