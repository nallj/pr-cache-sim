#ifndef _SIGNAL_CONTEXT_H_
#define _SIGNAL_CONTEXT_H_
...
class signalContext {
	...
	public:
		signalContext(std::vector<prrLevelController*>* prr_ctrls, prc* prc, icap* icap) :
				...
				stored_prr_prc_ack_(false), stored_prr_icap_ack_(false),
				stored_prc_start_prr_(false), stored_prc_enqueue_prr_(false), stored_prc_icap_req_(false),
				stored_icap_prc_ack_(false), stored_icap_transfer_(false), icap_current_trace_(nullptr) {
		...
		// changeSignal(); // not needed since they are pointers and will change already
		...
		/*void printContext() {
			...
			std::cout << "PRR_CTRLs: PRR_ICAP_ACK[" << stored_prr_icap_ack_ << "]\n";
			for (unsigned i = 0; i < prr_ctrl_count_; i++) {
				std::cout << "PRR_CTRL_" << i << ": "
						  << "PRR_EXE<" <<  stored_prr_executing_[i] << ">\n";
			}

			std::cout << "PRC: "
					  << "PRC_ICAP_REQ<" << stored_prc_icap_req_ << ">\n";
			for (unsigned i = 0; i < prr_ctrl_count_; i++) {
				std::cout << "PRC_PRR_CTRL_" << i << ": "
						  << "PRC_ENQUEUE_PRR<" <<  stored_prc_enqueue_prr_[i] << ">, "
						  << "PRC_START_PRR<" << stored_prc_start_prr_[i] << ">\n";
			}

			std::cout << "ICAP: "
					  << "ICAP_PRC_ACK<" << stored_icap_prc_ack_ << ">, "
					  << "ICAP_TRANSFER_PRR<" << stored_icap_transfer_ << ">\n";
			for (unsigned i = 0; i < prr_ctrl_count_; i++) {
				std::cout << "ICAP_PRR_CTRL_" << i << ": "
						  << "ICAP_PRR_REQ<" <<  stored_icap_prr_ctrl_req_[i] << ">\n";
			}



			std::cout << "<ENDING CONTEXT PRINT>\n";
		}*/
		...
		//std::deque<bool>* accessContextSignalBus(prcSignal signal) {
		...
		// ICAP_PRR_REQ
		// std::deque<bool>* accessContextSignalBus(icapSignal signal) {
	...
	private:
		std::string produceSpaces(unsigned count, unsigned var_len_num_count = 0, unsigned variable_length_num = 0) {
		...
		//std::deque<bool> stored_prr_icap_ack_;
