...
/* PUBLIC */
	...
	wallet::wallet() {
		...
		if (wallet_handler_.isFileValid()) {
			...
			for (it = wallet_contents.begin(); it != wallet_contents.end(); ++it) {
				...
				if (next[0].substr(0, 1) == "m") { // read in memory file
					...
					reconfigurableRegions new_mem = buildMemoryHierarchy(next[1]);
					memories_.push_back(new_mem);

				} else if(next[0].substr(0, 1) == "a") { // read in application file

					application* new_app = createApplication(next[1]);
					applications_.push_back(new_app);

				} else if(next[0].substr(0, 1) == "t") { // trace file list

					std::pair<std::string, std::string>* new_trace = createTraceFilePair(next[1]);
	...
	std::vector<std::string> wallet::getMemoryParamRules() { return memory_params_; }
	std::vector<std::string> wallet::getMemoryRegexRules() { return memory_regex_; }
	std::vector<std::string> wallet::getApplicationParamRules() { return application_params_; }
	std::vector<std::string> wallet::getApplicationRegexRules() { return application_regex_; }
	std::vector<std::string> wallet::getTraceParamRules() { return trace_params_; }
	std::vector<std::string> wallet::getTraceRegexRules() { return trace_regex_; }
	...
	std::vector<std::string> wallet::getTraceFile(unsigned trace_id) {
		fileHandler trace_reader = fileHandler(trace_files_[trace_id]->first, trace_params_, trace_regex_);
...
/* PRIVATE */
	...
	reconfigurableRegions wallet::buildMemoryHierarchy(std::string memory_file){
		...
		fileHandler memory_reader = fileHandler(memory_file, memory_params_, memory_regex_);
		...
		if (memory_reader.isFileValid()) {
			...
			std::multimap<std::string,std::string>::iterator it;
			for (it = memory_contents.begin(); it != memory_contents.end(); ++it) {
			...
			memoryLevel* main_memory = new memoryLevel(std::stol(main_mem_size), NULL,
					(unsigned short)std::stol(main_mem_read), (unsigned short)std::stol(main_mem_search));
			...
			if(highest_level > 0) {
				...
				for(int i = 1; i <= highest_level; i++){
					...
					memoryLevel* child = new memoryLevel(level_size[i], default_alg,
							level_read[i], level_search[i], lut_indexed[i]);
	...
	application* wallet::createApplication(std::string application_file) {
		...
		fileHandler application_reader = fileHandler(application_file, application_params_, application_regex_);

		if (application_reader.isFileValid()) {

			// start by retrieving the basic application parameters
			std::multimap<std::string, std::string> statically_defined_params = application_reader.getParams();

			// get the application's name
			std::string application_name = statically_defined_params.find("name")->second;

			// get icap speed and bus width, then instantiate the icap
			std::string icap_speed = statically_defined_params.find("icap speed")->second;
			std::string icap_width = statically_defined_params.find("icap width")->second;
			icap* application_icap = new icap(std::stod(icap_speed), std::stol(icap_width));

			// get prc speed, then instantiate the prc
			std::string prc_speed = statically_defined_params.find("prc speed")->second;
			prc* application_prc = new prc(std::stod(prc_speed));
			...
			double fastest_module_speed = std::stod(static_region_speed);

			std::multimap<unsigned, unsigned>* static_modules = new std::multimap<unsigned, unsigned>();
			std::multimap<unsigned, std::pair<unsigned, std::vector<double>>>* reconfigurable_modules
				= new std::multimap<unsigned, std::pair<unsigned, std::vector<double>>>();

			// start parsing the arbitrary amounts of static and reconfigurable regions
			std::vector<std::string> dynamically_declared_params = application_reader.getData();
			for(int i = 0; i < dynamically_declared_params.size(); i++){
				...
				if (param.substr(0, 2) == "sr") {
					...
					std::string region_id = param.substr(2, region_id_end_index - 2);
				...
				if (param.substr(0, 2) == "rr") {
					unsigned region_id_end_index = param.find(" ");
					unsigned region_id = std::stol(param.substr(2, region_id_end_index - 2));
					...
					if (param.substr(region_id_end_index + 1, 3) == "bit") {
						...
						std::pair<unsigned, std::pair<unsigned, std::vector<double>>> new_region
					...
					// get module speed
						...
						std::multimap<unsigned, std::pair<unsigned, std::vector<double>>>::iterator region_it;
						region_it = reconfigurable_modules->find(region_id);

						unsigned module_id_start_index = param.find("module") + 7;
						unsigned module_id_end_index = param.find(" ", module_id_start_index);
						std::string module_id = param.substr(module_id_start_index, module_id_end_index - module_id_start_index);
			...
			application* new_app = new application(application_icap, application_prc,
												   std::stod(static_region_speed), static_modules,
												   reconfigurable_modules, fastest_module_speed);
	...
	std::pair<std::string, std::string>* wallet::createTraceFilePair(std::string trace_file) {
		...
		fileHandler trace_reader = fileHandler(trace_file, trace_params_, trace_regex_);

		if (trace_reader.isFileValid()) {
			std::string trace_name = trace_reader.getParams().find("name")->second;
