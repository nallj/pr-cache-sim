...
class wallet{
	...
	private:
		...
		std::vector<std::string> memory_params_ {"name", "main name", "main size", "main read latency",
												"main search latency"};
		std::vector<std::string> memory_regex_ {"[l][1-9] \\bname\\b[:] [\\w ]+",
											    "[l][1-9] \\bsize\\b[:] [\\w ]+",
											   	"[l][1-9] \\b(read|search)\\b \\blatency\\b[:] [\\w ]+",
												"[l][1-9] has LUT"};

		std::vector<std::string> application_params_ {"name", "icap width", "icap speed", "prc speed",
													 "static region speed"};
		std::vector<std::string> application_regex_ {"\\bsr\\d+ \\bmodule count\\b[:] \\d+",
													"\\brr\\d+\\b \\bbitstream size\\b[:] \\d+",
													"\\brr\\d+\\b \\bmodule \\d+ speed\\b[:] \\d+[.]\\d{1,2}"};

		std::vector<std::string> trace_params_ {"name"};
		std::vector<std::string> trace_regex_ {//"s?(\\d+)(,\\s*\\d+){3},",
											   //"s?(\\d+)(,\\s*\\d+){4}"
												"s?(\\d+)(,\\d+){3},(\\d+)?"};
